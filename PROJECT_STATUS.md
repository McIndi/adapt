<!-- MILEMARKER: milestone=M1 lanes_ok=7/8 lag=0 updated=2026-09-01 -->
...
canonical location, and doing so surfaced a real chart bug (the `adapt`
release-name/service-link collision) rather than just documentation gaps
— see the Docs lane, which reads `WIP` this cycle pending that fix and
the live site publish. All of it is reflected in the lane table below;
see the per-lane notes for detail and for what's still open.
...
| 7 | Docs | WIP | README, the Diataxis-ish manual (`docs/manual/*`), the disclaimed spec (`docs/spec/*`), and `SECURITY.md`, now updated for persistence, uploads, admin-bootstrap, and NodePort. `RELEASING.md` documents independent `chart-v<version>` OCI releases and the canonical GHCR install command. Deployment documentation is now consolidated: a new `docs/manual/deployment.md` overview links a dedicated `container.md` (Docker/Podman, verified against the published multi-arch `0.5.0` image) and `kubernetes.md` (the single canonical Helm reference, with a day-1 walkthrough verified end-to-end, including an actual login). `README.md`, `docs/manual/installation.md`, and `charts/adapt/README.md` no longer carry three diverging copies of the same Helm content — each now points at `kubernetes.md`. `docs/manual/troubleshooting.md` gained a deployment section covering `fsGroup`/UID-1000 permission failures, startup-probe restarts, the discovery-is-startup-only gotcha, bootstrap-Job/PVC cleanup, and RWO node-affinity `ContainerCreating`. Verifying the walkthrough surfaced a real chart bug, not just a docs bug: the release name `adapt` collapses the chart's `fullname` to a bare Service named `adapt`, which Kubernetes then shadows with an auto-injected `ADAPT_PORT` service-link env var that collides with Adapt's own identically-named config variable, crash-looping the pod (`ADAPT_PORT must be an integer`) — documented as a warning in `kubernetes.md` and a `known_limitations.md` entry, with the walkthrough itself switched to release name `myadapt` to avoid it; the underlying chart behavior is not yet fixed (no `enableServiceLinks: false` escape hatch exists in the chart today) and is tracked as an open gap, not silently routed around. `mkdocs build --strict` passes locally, but the built site has not yet been published — `pages.yml` only runs on `release`/`workflow_dispatch`, and the last real deploy predates these docs changes; `Deployment` is not yet visible on the live site. Gap found during this review: `docs/manual/security.md` (the dedicated security-posture doc) was never updated for the upload feature's attack surface (path traversal, MIME sniffing, size limits) even though `api_reference.md`/`installation.md` do cover it as usage docs — this remains open. `CHANGELOG.md` still doesn't exist — targeted by M4. |
