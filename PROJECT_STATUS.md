<!-- MILEMARKER: milestone=M1 lanes_ok=8/8 lag=0 updated=2026-08-29 -->
...
| 4 | Packaging | OK | PyPI publish via CI with OIDC trusted publishing and the GHCR image publish path. The `Dockerfile` targets Python 3.14, restores dependency-layer caching, and carries OCI labels plus a `/health` `HEALTHCHECK`. The Helm chart is at release-candidate version 0.5.1-rc.1 with complete metadata and a values schema; the stable 0.5.1 will publish after the rc round trip passes. A tag-gated workflow now packages it for GHCR as `oci://ghcr.io/mcindi/charts/adapt`; a reviewer-found `tomllib`-on-YAML bug in the version-match step was fixed but not yet re-run, and no registry publication has happened — both pending Gate 5. The published 0.4.2 image manifest contains both amd64 and arm64. Still no SBOM/signing for either artifact type — targeted by M2. |
| 5 | Automation | OK | CI runs the test suite on every push/PR across a FastAPI version matrix, plus `dependency-audit` (`pip-audit`). `helm-ci.yml` runs for chart-affecting pushes to `main`, pull requests, manual dispatches, and reusable workflow callers, including `tools/test-helm-schema.sh` changes. `publish-chart.yml` calls the Python test and full Helm CI workflows before pushing a matching `chart-v<version>` package to GHCR. Its first registry run is pending Gate 5. Still no Python lint job, Dependabot, or backup/restore path — targeted by later milestones. |
