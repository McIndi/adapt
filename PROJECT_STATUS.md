...
| 4 | Packaging | OK | PyPI publish via CI with OIDC trusted publishing and the GHCR image publish path. The `Dockerfile` targets Python 3.14, restores dependency-layer caching, and carries OCI labels plus a `/health` `HEALTHCHECK`. The Helm chart is published as the stable `0.5.1` to `oci://ghcr.io/mcindi/charts/adapt`, with complete metadata and a values schema. A clean-machine round trip on a fresh VM — anonymous `helm show chart` plus `helm install` into a fresh kind cluster, reaching `HTTP 200` on `/health` — confirmed both the publish gating (full test suite plus Helm CI) and public package visibility for both the release-candidate and stable tags. The published 0.4.2 image manifest contains both amd64 and arm64. Still no SBOM/signing for either artifact type — targeted by M2. |
