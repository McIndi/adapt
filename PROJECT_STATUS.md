<!-- MILEMARKER: milestone=M1 lanes_ok=8/8 lag=0 updated=2026-08-21 -->
...
| 4 | Packaging | OK | PyPI publish via CI with OIDC trusted publishing and the GHCR image publish path. The `Dockerfile` targets Python 3.14, restores dependency-layer caching, and carries OCI labels plus a `/health` `HEALTHCHECK`. The Helm chart is now version 0.5.0 with complete metadata and a values schema. The published 0.4.2 image manifest contains both amd64 and arm64. Still no SBOM/signing for either artifact type — targeted by M2. |
| 5 | Automation | OK | CI runs the test suite on every push/PR across a FastAPI version matrix, plus `dependency-audit` (`pip-audit`). `helm-ci.yml` runs for chart-affecting pushes to `main`, pull requests, and manual dispatches, including `tools/test-helm-schema.sh` changes. Its first recorded Phase 1 run passed lint, 25 unit tests, and install smoke tests across three Kubernetes versions. Phase 4 adds schema validation checks, external volume passthrough, and notes coverage; VM verification passes lint, schema checks, chart unit tests, and live kind checks for ClusterIP, NodePort, Ingress, bootstrap Job retention, `kubectl cp`, and emitted namespace-qualified commands. Still no Python lint job, Dependabot, or backup/restore path — targeted by later milestones. |
