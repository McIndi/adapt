...
| 8 | Security | OK | The M0/M1 floor holds: no hardcoded secrets, bounded dependencies, a clean `pip-audit` gate, documented TLS assumptions, and a reporting policy. The navigation permission leak found during the deployment tracer is fixed and covered by regression tests. Bootstrap credentials are generated into a Kubernetes Secret, and publish tags must match package versions. Phase 1 adds the missing upload threat notes. Phase 3 inventories provenance and adds SBOMs plus missing image and chart integrity evidence. Secret scanning and SAST remain unscheduled. |
