...
name contains `adapt`. `helm install myadapt` therefore names the Service
`myadapt`, not `myadapt-adapt`. This is name shortening only. Server and
bootstrap pods set `enableServiceLinks: false`, so a release named `adapt`
does not inject `ADAPT_PORT` from the Service.
