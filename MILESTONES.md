...
| Packaging | `charts/adapt/` is versioned independently of the app via `Chart.yaml`; Phase 5 configures OCI publication to `oci://ghcr.io/mcindi/charts/adapt` from matching `chart-v<version>` tags. The chart is at release-candidate version 0.5.1-rc.1 for the first registry round trip; stable 0.5.1 follows once that passes. `appVersion` remains tied to the app release. The first registry publication is pending Gate 5. |
| Automation | `helm-ci.yml` runs for chart-affecting `main` pushes, pull requests, manual dispatches, and reusable workflow callers, including `tools/test-helm-schema.sh` changes. It runs lint, the chart unit tests, schema checks, and `kind` install smoke tests across 3 Kubernetes versions. `publish-chart.yml` calls it and the Python test workflow before a chart can be pushed to GHCR. |
