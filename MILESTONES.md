...
| Packaging | `charts/adapt/` is versioned independently of the app via `Chart.yaml`; Phase 5 published it to `oci://ghcr.io/mcindi/charts/adapt` from a matching `chart-v<version>` tag. The `chart-v0.5.1-rc.1` round trip passed, then the stable `chart-v0.5.1` was tagged and published the same way. `appVersion` remains tied to the app release. |
| Automation | `helm-ci.yml` runs for chart-affecting `main` pushes, pull requests, manual dispatches, and reusable workflow callers, including `tools/test-helm-schema.sh` changes. It runs lint, the chart unit tests, schema checks, and `kind` install smoke tests across 3 Kubernetes versions. `publish-chart.yml` calls it and the Python test workflow before a chart can be pushed to GHCR; it published both `chart-v0.5.1-rc.1` and the stable `chart-v0.5.1`, and a clean-machine `helm install` into a fresh kind cluster confirmed the package installs anonymously and serves `/health`. |
...
| Docs | README and `docs/manual/installation.md` gained a full Helm section: all three persistence modes, admin bootstrapping, NodePort, and a `values-dev.yaml` overlay for local cluster development. `RELEASING.md` documents chart tags and OCI installation. That content has since been consolidated: `docs/manual/deployment.md` is the entry point, `docs/manual/container.md` covers the published image, and `docs/manual/kubernetes.md` is the single canonical Helm reference (with a verified day-1 walkthrough); `README.md`, `installation.md`, and `charts/adapt/README.md` now point at it instead of duplicating it. |
